...
- 需求范围：仅新出现且标题精确包含全角标记`【新增】`的`Req`。
- 测试计划名称：移除`【新增】`和多余空白/标点，最多40字符；不得按标题猜需求ID。
- 日期：运行当天为开始日，结束日为开始日后14个自然日。
- 管理员、参与人：均为`谢佳伟`；每次写前用`SearchMembers`按启用状态精确唯一回读userId。
- 关联项目：`01_ONEOS`；关联迭代：需求返回的同一`sprint.id`。需求无迭代时不建计划，次日重试并通知。
- 用例库：扫描当前PAT可见的全部用例库。
...
1. 首次成功`--apply`只建立当前需求基线，默认不为历史`【新增】`需求补建计划。
2. 后续按需求内部ID去重；本地状态默认位于
   `%LOCALAPPDATA%\OneOS\YunxiaoQA\requirement-testplan-monitor.json`。
3. 创建前再按项目+迭代+规范化计划名调用`ListTestPlan`查重。
4. 创建描述写入`YunxiaoQA requirement=<需求内部ID>`幂等标识。
...
7. 只有整轮需求读取成功才更新`lastSuccessfulScanAt`。

`--bootstrap-existing`会处理首次运行时已有的匹配需求，属于扩大写入范围，必须另行
...
- 有匹配：状态`awaiting-case-confirmation`，通知计划ID/名称、需求编号、迭代ID、
...
```text
使用 YunxiaoQA 技能运行 skill-run yunxiao_requirement_plan_monitor.py --apply。
严格按技能中的“每日需求监测与测试计划”固定口径执行。若缺少PAT/组织ID，只报告
AUTH_CONFIG_MISSING和本机配置方法，不读取或写入云效；不要索要或回显令牌。若有新计划，
报告计划回读和用例匹配；匹配用例仅通知确认，不自动添加。若无新需求，报告本次扫描成功且零新增。
```
