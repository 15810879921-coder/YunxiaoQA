...
2. 与之正式关联的缺陷集合中，**每一条**状态 ∈ {`暂不修复`, `已关闭`}；每条`已关闭`都有自己的独立复测记录，每条`暂不修复`都有批准人和批准证据。
   集合来源：与【测试】**ASSOCIATED** 的缺陷（双向；兼容旧 PARENT 残留）。
3. 不存在 `待确认` / `处理中` / `已修复` / `再次打开` 等未闭环缺陷。

4. 已通过官方CLI回读当前 TestHub 计划；计划用例总数大于0，且未执行/失败/阻塞均为0。
5. 【测试】正式`TASK_SUB→【交付】`且`ASSOCIATED→需求`；需求当前为`测试中`；父【交付】已绑定同端有效迭代。

满足后：`完成测试`将【测试】`处理中→已完成`、需求`测试中→测试完成`，并回读两侧最终状态。

**唯一完整写入口**：`scripts/yunxiao_cli_test_lifecycle.py complete ... --test-plan-id <TestHub计划ID>`（先预检，确认后加`--apply`，官方CLI写入 + 两侧回读）。
**禁止**浏览器点「已完成」。
