...
发起缺陷：标题=…；描述=…；[测试任务=ONEOS-xx | 交付=ONEOS-xx]；[负责人=…]；[标签=…]；[附件=文件1,文件2]
从测试用例发起缺陷：测试用例=CASE-xx；标题=…；描述=…；[测试任务=ONEOS-xx]；[负责人=…]；[标签=…]；[附件=…]
发起缺陷(非本期)：标题=…；描述=…；负责人=…；[标签=…]；[附件=…]；[项目=…]
...
- 默认状态：`已分配` + `处理中`（`待处理`仅兼容历史任务，可显式指定）。
...
- 可按测试任务 / 负责人过滤。
- 只读；可不进 Plan。
- 回归通过必须评论测试结果并关闭；仍复现必须上传证据（若有）、评论并再次打开；未判定不写云效。
...
- `开始测试`同时推进【测试】`已分配→处理中`和需求`待测试→测试中`。
...
- `完成测试`须校验关系、证据清单、用例计数闭合、逐Bug复测证据与暂不修复批准，同时依次推进【测试】`处理中→已完成`、需求`测试中→测试完成`和父【交付】活动态→`已完成`。
...
- 回报必须含开始阶段两侧、完成阶段三侧的`serialNumber | subject | from→to`；任一回读不符则停。
- **禁止**浏览器点状态菜单 / 可交互节点。
- 写操作 → 必须 Plan。
