...
class FakeClient:
    ...
    def search_deliveries(self, project_id: str, marker: str, max_pages: int) -> list[dict[str, object]]:
...
class DeliveryPlanMonitorTests(unittest.TestCase):
    def test_exact_full_width_marker_is_required(self) -> None:
        self.assertTrue(monitor.qualifies("【交付】【新增】云打印"))
        self.assertFalse(monitor.qualifies("【新增】云打印"))
        self.assertFalse(monitor.qualifies("【交付】新增云打印"))
        self.assertFalse(monitor.qualifies("【开发】【新增】云打印"))

    def test_plan_name_is_simplified_and_limited(self) -> None:
        name = monitor.normalize_plan_name(" 【交付】【新增】  【PC】  云打印能力优化  ")
    ...
    def test_case_keywords_include_specific_core(self) -> None:
        self.assertEqual(monitor.case_keywords("【交付】【新增】支持云打印功能"), ["支持云打印功能", "云打印"])
    ...
    def test_missing_sprint_blocks_without_create(self) -> None:
        client = FakeClient()
        result = monitor.process_delivery(
            client,
            {"id": "delivery-1", "serialNumber": "ONEOS-1", "subject": "【交付】【新增】云打印"},
            project_id=monitor.DEFAULT_PROJECT_ID,
            manager_id="user-1",
            apply=True,
        )
    ...
    def test_first_apply_builds_baseline_without_creating_historical_plan(self) -> None:
        ...
        self.assertEqual(client.created, [])
        self.assertEqual(state["deliveries"]["delivery-old"]["status"], "baseline")

    def test_known_delivery_is_idempotently_skipped(self) -> None:
        delivery = {
            "id": "delivery-1",
            "serialNumber": "ONEOS-1",
            "subject": "【交付】【新增】云打印",
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            monitor.write_json(state_path, {
                "schemaVersion": "oneos.yunxiao-delivery-plan-monitor/v1",
                "initializedAt": "2026-08-19T00:00:00+08:00",
                "deliveries": {"delivery-1": {"status": "no-matching-cases"}},
            })
            result = monitor.run(args_for(state_path, root / "receipt.json"), client)
        self.assertEqual(result["newDeliveryTaskCount"], 0)
        self.assertEqual(client.created, [])
