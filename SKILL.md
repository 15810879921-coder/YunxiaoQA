...
## 缺陷人话版描述与源码测试范围（用户固定规则）

- 每次新建缺陷（独立、测试用例、非本期或其他建单入口），缺陷正文必须包含标题明确且内容非空的「人话版描述」段。用几句日常语言写清在哪里、做了什么、实际出了什么问题、应该怎样；只写已确认的影响，不把推测原因或未确认影响写成事实。技术证据仍保留在其他章节。
- 「人话版描述」必须随缺陷正文持久化，不能只出现在聊天回执、Plan、标题、评论或占位文案中。apply 前检查，创建后回读描述并核对该段存在且与证据一致；缺失、空白或不一致即停止，不得声称提单完成。
- 测试或缺陷诊断需要读取代码时，路径中目录名恰为 `src.test` 的代码不纳入测试目标：不为其设计或执行测试，也不把未覆盖它记为缺口。必要时可只为理解相邻业务读取；除非用户本次明确要求，不得把该目录加入测试范围。目录外的真实业务行为、接口与回归仍照常验证。
...
- [ ] 缺陷正文：持久化正文含非空「人话版描述」，且创建后已回读并与证据一致
- [ ] 源码测试范围：若读取代码，已排除目录名恰为 src.test 的代码；未把该排除记为缺口
