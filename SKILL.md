...
description: >-
  ...
  从TestHub回读计划与用例执行结果，诊断查重后一键发起缺陷；
  ...
  可走人工快捷闭环，使测试任务与正式关联需求进入完成态；闭环发现父【交付】缺迭代时，
...
人工快捷闭环 = 明确编号+用户确认人工验证通过；允许跳过TestHub，但仍校验正式关系、活动缺陷和逐步回读；不产出发布候选
...
发布候选真相 = 完成态【测试】+ TestHub计划全量通过 + 测试完成需求 + 正式关系 + 缺陷闭环
...
4. **开始/完成测试**：只用 `skill-run yunxiao_cli_test_lifecycle.py start|complete|manual-complete ...`；逐Bug复测关闭只用 `skill-run yunxiao_cli_bug_retest.py ...`。常规`complete`直接回读TestHub计划结果与缺陷闭环，不要求test部署记录、正式测试报告或`oneos.qa-evidence/v1`清单/哈希；`manual-complete`仅在用户针对明确编号确认人工验证通过后使用，跳过TestHub门禁但仍校验正式关系、无活动缺陷和写后回读，且不产出发布候选。`skill-run` 按 [references/runtime-launcher.md](references/runtime-launcher.md) 解析。
...
4. 该路径不要求TestHub，但必须按状态机补齐中间态并逐步回读：任务最终=`已完成`、需求最终=`测试完成`。
...
开始、记录或完成测试（含人工快捷闭环）时，必须回读测试任务唯一父【交付】的`sprint`：
...
6. **禁止**浏览器点状态；**禁止**在仍有活跃缺陷时改需求；**不**因此放宽完整`完成测试`的 TestHub/缺陷门禁——本口令是「测试任务已完成且无缺陷」后的需求回写捷径。
...
4. 直接通过官方CLI回读指定TestHub计划：计划内用例总数`>0`，未执行、失败、阻塞均为0；不要求正式测试报告ID/链接。
5. 无`待确认/处理中/已修复/再次打开`缺陷；每条已关闭Bug均带独立`oneos.bug-retest/v1`复测通过证据。
6. 每条`暂不修复`缺陷都有明确批准人和证据；只有状态没有批准证据仍阻塞。
7. 门禁通过后依次推进【测试】→`已完成`、需求→`测试完成`并逐项回读。
8. 输出项目、迭代、需求、交付、测试任务、TestHub计划、缺陷状态、完成时间和幂等键，交给`$yunxiao-release-operations`组建发布批次。
...
- [ ] 完成测试：TestHub计划非空且全部通过；关联缺陷全部闭环；暂不修复均有批准证据；未要求部署记录、正式报告或QA manifest/哈希
